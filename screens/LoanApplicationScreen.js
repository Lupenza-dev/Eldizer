...
const LoanApplicationScreen = ({ route }) => {
  ...
  const loanApplicationMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!bankId) throw new Error(t("bank_name"));
...
console.log('error',error);
        error.response?.data?.errors || error.message || "Application failed";
  ...
  const submitGuarantor = (index) => {
    ...
    if (!g.fs)
      return Toast.show({
        type: "error",
        text1: t("guarantor_fullname_required"),
      });
    if (!g.rs)
      return Toast.show({
        type: "error",
        text1: t("guarantor_relationship_required"),
      });
    if (!g.pn)
      return Toast.show({
        type: "error",
        text1: t("guarantor_phone_required"),
      });
        ...
        <ScrollView contentContainerStyle={styles.scrollViewContent}>
          <View style={styles.container}>
            ...
            <View style={styles.middleView}>
              ...
              <DetailRow
                label={loan_type == 2 ? t("request_loan") : t("request_amount")}
                value={`${request_amount.toLocaleString()} TZS`}
              />
              {loan_type == 2 && (
                <DetailRow
                  label={t("initial_deposit")}
                  value={`${initial_deposit.toLocaleString()} TZS`}
                />
              )}
            ...
            <View style={{ margin: 10 }}>
              ...
              <Dropdown
                ...
                renderRightIcon={() => (
                  <AntDesign color="black" name="down" size={12} />
                )}
              />
              <Dropdown
                style={styles.dropdown}
...
const GuarantorButton = ({ added, label, onPress, marginTop = 0 }) => (
  <TouchableOpacity
    activeOpacity={0.9}
    style={[styles.guarantorView, { marginTop }]}
    onPress={onPress}
  >
    ...
    <View style={styles.bottomNavigationView}>
      <View style={{ margin: 10 }}>
        ...
        <FormInput
          label="Full Name"
          value={guarantor.fs}
          onChangeText={(text) => setGuarantor({ ...guarantor, fs: text })}
        />
        <FormInput
          label="Relationship"
          value={guarantor.rs}
          onChangeText={(text) => setGuarantor({ ...guarantor, rs: text })}
        />
        <FormInput
          label="Phone Number"
          value={guarantor.pn}
          onChangeText={(text) => setGuarantor({ ...guarantor, pn: text })}
        />
        <View style={{ marginTop: 20 }}>
          <IconButton
            icon="arrow-circle-right"
            name="Submit Guarantor"
            onPress={onSubmit}
          />
        </View>
      </View>
